...
public class RedisPoolsUtil {
    ...
    private static RedisPoolsUtil redisPoolsUtil = null;
    ...
    //https://www.cnblogs.com/fanwencong/p/5782860.html

    public static RedisPoolsUtil getSingleton() {
        if (redisPoolsUtil == null) {
            return new RedisPoolsUtil();
        }
        return redisPoolsUtil;
    ...
    public void doTest() {
        ...
        // 创建连接池
        JedisPool pool = new JedisPool(config, "101.200.87.141", 6379, 200000);
        ...
        // for (int i = 1; i <= 50; i++) {//这里自己设置用多少线程并发访问
        //     String teacherName = String.valueOf(i);
        //     new ThreadToMysql(teacherName, "123456", pool, teacherMapper).start();
        // }
    }


    private RedisPoolsUtil() {

        if (systemConfig == null) {
            systemConfig = ApplicationUtil.getBean(SystemConfig.class);
        }
        if (teacherMapper == null) {
            teacherMapper = ApplicationUtil.getBean(TeacherMapper.class);
        }

        if (pool == null) {
            String ip = systemConfig.getRedisIp();
     ...
     * 通过key获取set中的差集
     * 以第一个set为标准
     * s 可以 是一个string 则返回set中所有的value 也可以是string数组
     *
     * @return 返回值
     */
     ...
     *
     * @param dstkey s   可以 是一个string 也可以是一个string数组
     * @return 返回值
     */
    ...
    private Jedis getJedis() {
        return redisPoolsUtil.pool.getResource();
    }
